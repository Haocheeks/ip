...
/**
 * Tests {@link LogBook#listTaskDueBy(LocalDateTime)}.
 *
...
public class LogBookTest {
    ...
    @Test
    public void listTaskDueBy_taskDueBeforeCutoff_taskListed() throws HermesException {
        logBook.log(new Deadline("essay", LocalDateTime.of(2026, 8, 29, 9, 0)));

        assertEquals("[D][ ] essay (by: 29 Aug 2026 0900)", logBook.listTaskDueBy(CUTOFF));
    }

    @Test
    public void listTaskDueBy_taskDueAfterCutoff_taskExcluded() throws HermesException {
        logBook.log(new Deadline("far future", LocalDateTime.of(2026, 12, 25, 9, 0)));

        assertEquals("Nothing is due by 30 Aug 2026 1200", logBook.listTaskDueBy(CUTOFF));
    }

    @Test
    public void listTaskDueBy_taskDueExactlyAtCutoff_taskListed() throws HermesException {
        // The cutoff is inclusive: a task due at the very moment asked about counts.
        logBook.log(new Deadline("on the dot", CUTOFF));

        assertEquals("[D][ ] on the dot (by: 30 Aug 2026 1200)", logBook.listTaskDueBy(CUTOFF));
    }

    @Test
    public void listTaskDueBy_completedTask_taskExcluded() throws HermesException {
    ...
    @Test
    public void listTaskDueBy_todoWithoutDate_taskExcluded() throws HermesException {
    ...
    @Test
    public void listTaskDueBy_severalTasksDue_listedSoonestFirst() throws HermesException {
        ...
        assertEquals("""
                ...
                logBook.listTaskDueBy(CUTOFF));
    }

    @Test
    public void listTaskDueBy_emptyLogBook_noticeReturned() {
        assertEquals("Nothing is due by 30 Aug 2026 1200", logBook.listTaskDueBy(CUTOFF));
    }

    @Test
    public void listTaskDueBy_queryRun_listOrderUnchanged() throws HermesException {
        logBook.log(new Deadline("essay", LocalDateTime.of(2026, 8, 29, 9, 0)));
        logBook.log(new Deadline("old", LocalDateTime.of(2026, 8, 25, 8, 0)));
